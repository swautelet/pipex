...
/*   Updated: 2022/03/15 14:44:05 by swautele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_here_doc(int argc, char **argv, char **envp)
{
	...
	if (argc < 5)
		return (0);
	r.i = 2;
	r.fd[r.i] = open("/temp/iwehiuwfiewchu.tmp", O_CREAT | O_WRONLY | O_TRUNC, 00777);
	if (r.fd[r.i] == -1)
		exit_error("failed to create tmp file");
	wr_heredoc(r.fd[r.i], argv[r.i]);
	close (r.fd[r.i]);
	r.fd[r.i] = open("/temp/iwehiuwfiewchu.tmp", O_RDONLY, 00777);
	unlink("/temp/iwehiuwfiewchu.tmp");
